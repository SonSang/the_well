...
class UNetConvNextTAWM(nn.Module):
    def __init__(
        self,
        dim_in: int,
        dim_out: int,
        ...
        super(UNetConvNextTAWM, self).__init__()
        ...
        decoder_dims = [(features * 2**i)+1 for i in range(stages, -1, -1)] # account for dt_channel
        encoder = []
        decoder = []
        ...
        self.out_proj = conv_modules[n_spatial_dims](
            features+2, dim_out, kernel_size=3, padding=1 # account for dt_channel
        )
    ...
    """
        Euler formulation with ∆t-conditioning UNet:
            x{t+1} = x{t} + UNet(xs, dt) * log(dt)
            
        where:
            + UNet encoder blocks: no modification -> x = Enc(x)
            + UNet decoder blocks: ∆t-conditioning -> x = Dec(x, x_skip, dt)
    """
    def forward(self, x, dt, F):
        # raw input: [x_{t-H}, ..., x_t] => extract only original x_t
        xt_input = x.detach().clone()[:,-F:,:,:]
        device = x.device
        ...
        for j, dec in enumerate(self.decoder):
            ...
            # ∆t-conditioning in latent space
            # print('Latent shape:', x.shape)
            B, D, H, W = x.shape
            dt_channel = torch.full((B, 1, H, W), fill_value=float(dt)).to(device)
            # decoder stage
            x = torch.cat([x, dt_channel], dim=1)
            x = dec(x)
            
        # Euler formulation: x' = x{t} + UNet(xs,dt) * log(dt)
        B, D, H, W = x.shape
        dt_channel = torch.full((B, 1, H, W), fill_value=float(dt)).to(device)
        x = torch.cat([x, dt_channel], dim=1)
        x = xt_input + self.out_proj(x) * math.log10(dt+1)
        return x